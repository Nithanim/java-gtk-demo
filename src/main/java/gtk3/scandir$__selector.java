// Generated by jextract

package gtk3;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import jdk.incubator.foreign.*;
import static jdk.incubator.foreign.ValueLayout.*;
public interface scandir$__selector {

    int apply(jdk.incubator.foreign.MemoryAddress _x0);
    static NativeSymbol allocate(scandir$__selector fi, ResourceScope scope) {
        return RuntimeHelper.upcallStub(scandir$__selector.class, fi, constants$112.scandir$__selector$FUNC, "(Ljdk/incubator/foreign/MemoryAddress;)I", scope);
    }
    static scandir$__selector ofAddress(MemoryAddress addr, ResourceScope scope) {
        NativeSymbol symbol = NativeSymbol.ofAddress("scandir$__selector::" + Long.toHexString(addr.toRawLongValue()), addr, scope);
return (jdk.incubator.foreign.MemoryAddress _x0) -> {
            try {
                return (int)constants$112.scandir$__selector$MH.invokeExact(symbol, (jdk.incubator.foreign.Addressable)_x0);
            } catch (Throwable ex$) {
                throw new AssertionError("should not reach here", ex$);
            }
        };
    }
}


