...
public interface g_thread_gettime {
    ...
    static NativeSymbol allocate(g_thread_gettime fi, ResourceScope scope) {
        return RuntimeHelper.upcallStub(g_thread_gettime.class, fi, constants$344.g_thread_gettime$FUNC, "()J", scope);
    }
...
return () -> {
            try {
                return (long)constants$344.g_thread_gettime$MH.invokeExact(symbol);
            } catch (Throwable ex$) {
