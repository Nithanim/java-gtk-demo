// Generated by jextract

package gtk3;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import jdk.incubator.foreign.*;
import static jdk.incubator.foreign.ValueLayout.*;
class constants$649 {

    static final FunctionDescriptor g_icon_new_for_string$FUNC = FunctionDescriptor.of(Constants$root.C_POINTER$LAYOUT,
        Constants$root.C_POINTER$LAYOUT,
        Constants$root.C_POINTER$LAYOUT
    );
    static final MethodHandle g_icon_new_for_string$MH = RuntimeHelper.downcallHandle(
        "g_icon_new_for_string",
        constants$649.g_icon_new_for_string$FUNC, false
    );
    static final FunctionDescriptor g_icon_serialize$FUNC = FunctionDescriptor.of(Constants$root.C_POINTER$LAYOUT,
        Constants$root.C_POINTER$LAYOUT
    );
    static final MethodHandle g_icon_serialize$MH = RuntimeHelper.downcallHandle(
        "g_icon_serialize",
        constants$649.g_icon_serialize$FUNC, false
    );
    static final FunctionDescriptor g_icon_deserialize$FUNC = FunctionDescriptor.of(Constants$root.C_POINTER$LAYOUT,
        Constants$root.C_POINTER$LAYOUT
    );
    static final MethodHandle g_icon_deserialize$MH = RuntimeHelper.downcallHandle(
        "g_icon_deserialize",
        constants$649.g_icon_deserialize$FUNC, false
    );
    static final FunctionDescriptor g_emblem_get_type$FUNC = FunctionDescriptor.of(Constants$root.C_LONG_LONG$LAYOUT);
    static final MethodHandle g_emblem_get_type$MH = RuntimeHelper.downcallHandle(
        "g_emblem_get_type",
        constants$649.g_emblem_get_type$FUNC, false
    );
    static final FunctionDescriptor g_emblem_new$FUNC = FunctionDescriptor.of(Constants$root.C_POINTER$LAYOUT,
        Constants$root.C_POINTER$LAYOUT
    );
    static final MethodHandle g_emblem_new$MH = RuntimeHelper.downcallHandle(
        "g_emblem_new",
        constants$649.g_emblem_new$FUNC, false
    );
    static final FunctionDescriptor g_emblem_new_with_origin$FUNC = FunctionDescriptor.of(Constants$root.C_POINTER$LAYOUT,
        Constants$root.C_POINTER$LAYOUT,
        Constants$root.C_INT$LAYOUT
    );
    static final MethodHandle g_emblem_new_with_origin$MH = RuntimeHelper.downcallHandle(
        "g_emblem_new_with_origin",
        constants$649.g_emblem_new_with_origin$FUNC, false
    );
}


